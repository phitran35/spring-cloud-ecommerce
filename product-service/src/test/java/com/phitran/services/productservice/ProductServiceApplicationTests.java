...
import com.phitran.services.productservice.helper.ProductFactory;
...
class ProductServiceApplicationTests {
    ...
    public void initDataInDB() {
        if (!isInitializedDB) {
            ...
            Product product6 = ProductFactory.samsungProduct().name("galaxy S10").color("Black").price(BigDecimal.valueOf(500)).id(6L).build();
            Product product7 = ProductFactory.galaxyS20Phone().id(7L).build();
            Product product5 = ProductFactory.samsungProduct().name("galaxy Tab").color("White").price(BigDecimal.valueOf(700)).id(5L).build();
            Product product3 = ProductFactory.appleProduct().name("iPod").color("White").price(BigDecimal.valueOf(999)).id(3L).build();
            Product product1 = ProductFactory.iphone().id(1L).build();
            Product product2 = ProductFactory.appleProduct().name("iPad").color("Black").price(BigDecimal.valueOf(1500)).id(2L).build();
            Product product4 = ProductFactory.appleProduct().name("iMac").color("White").price(BigDecimal.valueOf(2000)).id(4L).build();
            Product product8 = ProductFactory.kiaProduct().name("Cerato").color("Black").price(BigDecimal.valueOf(400000.00)).id(8L).build();
            Product product9 = ProductFactory.kiaProduct().name("Optima").color("White").price(BigDecimal.valueOf(500000.00)).id(9L).build();
    ...
    void getAllProductsWhenUserLoggedInThenReturnCorrectResult() {
        ...
        List<Product> products = restTemplate.exchange(PRODUCT_PATH,
    ...
    void getProductsByAppleBrandWhenUserLoggedInThenReturnCorrectResult() {
        ...
        UriComponentsBuilder uriBuilder = UriComponentsBuilder.fromHttpUrl("http://localhost:" + port + PRODUCT_PATH)
    ...
    void updateProductNameThenReturnUpdatedProductAndNoKeepTrackProductHistoryRecord() throws JsonProcessingException {
        Product product = ProductFactory.iphone().build();
    ...
    private Product sendPutRequestToUpdateProduct(Product updatedProduct) throws JsonProcessingException {
        ...
        ResponseEntity<Product> responseEntity =
                restTemplate.exchange(PRODUCT_PATH + "/" + updatedProduct.getId().toString(),
    ...
    void updateProductPriceThenReturnUpdatedProductAndKeepTrackProductHistoryRecord() throws JsonProcessingException {
        ...
        Product product = ProductFactory.samsungProduct().name("galaxy S10").color("Black").price(BigDecimal.valueOf(501)).id(6L).build();
